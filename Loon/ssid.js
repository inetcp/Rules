...
class ModelRegulator {
    ...
    /**
     * 配置蜂窝网络的运行模式
     * @param {Object} cellularModel 
     * @returns this
     */
    setCellularModel(cellularModel) {
        if (typeof cellularModel?.runningModel === "string")
            cellularModel.runningModel = RunningModel[cellularModel.runningModel]
        this.cellularModel = cellularModel
        return this
    }
    ...
    _getModel() {
        const model = this.config.ssid ? this.modelList[this.config.ssid] : this.cellularModel;
    ...
    changeModel(callback) {
        ...
        if (this._isNotification()) {
            $notification.post("网络变化", `网络已切换到：${config.ssid || "蜂窝网络"}`, message)
        }
...
dc.setKeyPrefix("inetcp.boxjs.ssid.")
...
const userStorage = {
    ...
    cellularModel: dc.getData("cellularModel", {}, true),
...
modelRegulator
    ...
    .setCellularModel(userStorage.cellularModel)
